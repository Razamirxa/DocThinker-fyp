...
from routes import loginPage, signupPage, profilePage, analyticsDashboard, uploadBooksPage, notificationPage, userManagement, chatRoutes, ragRoutes
...
app.include_router(ragRoutes.router, prefix="/api", tags=["rag"])
...
async def startup_event():
    ...
    # Initialize RAG engine with API keys from environment
    try:
        from rag_engine import rag_engine
        
        if settings.GOOGLE_API_KEY and settings.PINECONE_API_KEY:
            print("\n🔧 Initializing RAG Engine...")
            success, message = rag_engine.initialize(
                google_api_key=settings.GOOGLE_API_KEY,
                pinecone_api_key=settings.PINECONE_API_KEY,
                model_name=settings.DEFAULT_RAG_MODEL
            )
            
            if success:
                print(f"✅ RAG Engine initialized successfully!")
                print(f"📚 Model: {settings.DEFAULT_RAG_MODEL}")
                print(f"🗂️  Pinecone Index: {settings.PINECONE_INDEX_NAME}")
                print(f"📖 Available subjects: Linear Algebra, Discrete Structures, Calculus")
            else:
                print(f"⚠️  RAG Engine initialization failed: {message}")
        else:
            print("⚠️  RAG Engine not initialized: API keys not found in environment")
    except Exception as e:
...
def update_profile(
    ...
    if avatar:
        ...
        if kind is None or kind.extension not in ("jpeg", "jpg", "png", "gif"):

...
async def chat(
    ...
    try:
        ...
        if not rag_engine.initialized:
            return {
                "answer": "⚠️ The knowledge base is not initialized yet. Please contact the administrator to set up the system with API keys.",
                "type": "error"
            }
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s)
        """, ("rag_chat", current_user["id"], "asked", question[:100]))
        db.commit()
        
        # Process with RAG engine - works with Linear Algebra, Discrete Math, and Calculus
        result = rag_engine.answer_general_query(question)
        
        return {
            "answer": result.get("answer", "I couldn't find relevant information in the documents."),
